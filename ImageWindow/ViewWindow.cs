...
namespace ViewWindow
{
    public class ViewWindow : Model.IViewWindow
    {
        ...
        /// <summary>
        /// 监听 ROI 控制器的创建、激活、移动和完成更新事件。
        /// 使用控制器事件可避免依赖 HMouseEventArgs.Button 在拖动期间的兼容性差异。
        /// </summary>
        public void AddRoiObserver(Model.IconicDelegate observer)
        {
            if (observer != null)
                _roiController.NotifyRCObserver += observer;
        }
                ...
                switch (rois[index].Type)
                {
                    ...
                    case "ROIRectangle2":
                        ...
                        if (m_roiData != null)
                        {
                            ...
                            Model.ROIRectangle2 sourceRectangle = rois[index] as Model.ROIRectangle2;
                            Model.ROIRectangle2 displayedRectangle = this._roiController.getActiveROI() as Model.ROIRectangle2;
                            if (sourceRectangle != null && displayedRectangle != null)
                                displayedRectangle.EndpointRotationEnabled = sourceRectangle.EndpointRotationEnabled;
                        }
                        break;
                    case "ROICircle":
            ...
            foreach (var roi in rois)
            {
                ...
                m_roiData = roi.getModelData();
                        if (m_roiData != null)
                        {
                            ...
                            Model.ROIRectangle2 sourceRectangle = roi as Model.ROIRectangle2;
                            Model.ROIRectangle2 displayedRectangle = this._roiController.getActiveROI() as Model.ROIRectangle2;
                            if (sourceRectangle != null && displayedRectangle != null)
                                displayedRectangle.EndpointRotationEnabled = sourceRectangle.EndpointRotationEnabled;
        ...
        /// <summary>
        /// 使用调用方传入的 ROI 实例作为窗口中的交互对象，不创建显示副本。
        /// 适用于需要在拖动过程中持续读取最新坐标的工具。
        /// </summary>
        public void displayInteractiveROI(List<Model.ROI> rois)
        {
            ...
            foreach (Model.ROI roi in rois)
            {
                if (roi != null)
                    _roiController.ROIList.Add(roi);
            }

            _roiController.activeROIidx = _roiController.ROIList.Count - 1;
            _hWndControl.repaint();
        }
