...
namespace ViewWindow.Model
{
	...
	public class HWndCtrl
	{
        ...
        public bool EnableImagePan = true;
		...
		/// <summary>
		/// 按图像坐标范围(row1,col1,row2,col2为HALCON Part的左上/右下角坐标)恢复显示区域，
		/// 用于保留用户缩放/平移视图。ImagePart 的 Width/Height 为像素数量(内部SetPart会-1)，
		/// 因此宽度取 col2-col1+1；ImgRow2/ImgCol2 按"起点+像素数"同步，保证滚轮缩放计算一致。
		/// 注意：此处不修改 imageWidth/imageHeight，它们必须保持为背景图像的完整尺寸，
		/// 供 FitImagePartToWindow(适应窗口) 正确计算等比适配范围。
		/// </summary>
		public void SetViewPart(int row1, int col1, int row2, int col2)
		{
			int viewWidth = col2 - col1 + 1;
			int viewHeight = row2 - row1 + 1;

			ImgRow1 = row1;
			ImgCol1 = col1;
			ImgRow2 = row1 + viewHeight;
			ImgCol2 = col1 + viewWidth;
		...
		/// <summary>
		/// 按窗口宽高比等比显示整幅图像。窗口多出的区域使用图像坐标范围外的留白，
		/// 避免横图旋转为竖图后被强制铺满窗口而产生非等比拉伸。
		/// </summary>
		private void FitImagePartToWindow()
		{
			int viewportWidth = Math.Max(1, viewPort.ClientSize.Width);
			int viewportHeight = Math.Max(1, viewPort.ClientSize.Height);
			if (imageWidth <= 0 || imageHeight <= 0)
				return;

			double imageAspect = (double)imageWidth / imageHeight;
			double windowAspect = (double)viewportWidth / viewportHeight;
			double partWidth;
			double partHeight;
			...
			if (imageAspect > windowAspect)
			{
				...
				partHeight = imageWidth / windowAspect;
			}
			else
			{
				partHeight = imageHeight;
				partWidth = imageHeight * windowAspect;
			}

			ImgCol1 = (imageWidth - partWidth) / 2.0;
			ImgRow1 = (imageHeight - partHeight) / 2.0;
			ImgCol2 = ImgCol1 + partWidth;
			ImgRow2 = ImgRow1 + partHeight;
			zoomWndFactor = partWidth / viewportWidth;

			System.Drawing.Rectangle rect = viewPort.ImagePart;
			rect.X = (int)Math.Floor(ImgCol1);
			rect.Y = (int)Math.Floor(ImgRow1);
			rect.Width = Math.Max(1, (int)Math.Ceiling(partWidth));
			rect.Height = Math.Max(1, (int)Math.Ceiling(partHeight));
			viewPort.ImagePart = rect;
		}
		...
		private void mouseUp(object sender, HalconDotNet.HMouseEventArgs e)
		{
			...
			if (roiManager != null
				...
				&& (dispROI == MODE_INCLUDE_ROI))
			{
				// MouseMove 可能被系统合并；松开时提交鼠标的最终坐标。
				if (mousePressed)
