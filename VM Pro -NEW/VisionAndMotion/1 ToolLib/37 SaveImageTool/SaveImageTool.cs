...
namespace VMPro
{
    ...
    internal class SaveImageTool : ToolBase
    {
        internal SaveImageTool()
        {
            ...
            imageSavePath = string.Format("D:\\VM Pro\\Image\\{0}\\原始图像", jobname);
        }
        ...
        internal void ResetTool()
        {
            try
            {
                ...
                Frm_SaveImageTool.Instance.textBox1.Text = saveDays.ToString();
        ...
        public override void Run(bool updateImage, bool b, string toolName)
        {
            try
            {
                lock (obj)
                {
                    ...
                    if (expandTime)
                    {
                        fileName = imageSavePath + directoryName + "\\图像 " + DateTime.Now.ToString("HH_mm_ss ffff") + "." + imageFormat;
                    }
                    else
                    {
                        int index;
                        ...
                        {
                            fileName = imageSavePath + directoryName + "\\图像" + index + "." + imageFormat;
