...
namespace VMPro
{
    ...
    public class ToolInfo
    {
        ...
        public ToolIO GetOutput(string IOName)
        {
            ...
            string normalizedIOName = IOName == null ? string.Empty : IOName.Trim();
            for (int i = 0; i < output.Count; i++)
            {
                string outputName = output[i].IOName == null ? string.Empty : output[i].IOName.Trim();
                if (outputName == normalizedIOName)
                    return output[i];
            }

            if (normalizedIOName == "图像" || normalizedIOName == "输出图像" || normalizedIOName == "OutputImage")
            {
                for (int i = 0; i < output.Count; i++)
                {
                    string outputName = output[i].IOName == null ? string.Empty : output[i].IOName.Trim();
                    if (outputName == "图像" || outputName == "输出图像" || outputName == "OutputImage")
                        return output[i];
                }
            }
