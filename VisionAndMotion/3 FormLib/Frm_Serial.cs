...
namespace VMPro
{
    internal partial class Frm_Serial : Form
    {
        internal Frm_Serial()
        {
            InitializeComponent();
        ...
        public static Frm_Serial Instance
        {
            get
            {
        ...
        internal void LoadPar(Serial scaner_1)
        {
            ...
            {
                serial = scaner_1;
                tbx_clientName.TextStr = serial.Name;
                bool contains = false;
                for (int i = 0; i < cbx_portName.Items.Length; i++)
                {
                    if (cbx_portName.Items[i] == serial.portName)
                    {
                        contains = true;
                        break;
                    }
                ...
                if (!contains)
                    cbx_portName.Add(serial.portName);
                cbx_portName.TextStr = serial.portName;
                cbx_baudRate.TextStr = serial.baudRate.ToString();
                tbx_dataBit.TextStr = serial.dataBit.ToString();
                cbx_stopBit.SelectedIndex = (int)serial.stopBit;
                cbx_parityBit.SelectedIndex = (int)serial.parity;

                tbx_trigCmd.TextStr = serial.TrigCmd;
                tbx_scanNum.TextStr = serial.failNum.ToString();
        ...
        private void cbx_portName_SelectedIndexChanged()
        {
            ...
            serial.portName = cbx_portName.TextStr.Trim();
        }
        private void cbx_baudRate_SelectedIndexChanged()
        {
            try
            {
                ...
                serial.baudRate = Convert.ToInt32(cbx_baudRate.TextStr.Trim());
            }
            catch { }
        }
        private void tbx_dataBit_SelectedIndexChanged()
        {
            try
            {
                ...
                serial.dataBit = Convert.ToInt32(tbx_dataBit.TextStr.Trim());
            }
            catch { }
        }
        private void cbx_stopBit_SelectedIndexChanged()
        {
            ...
            serial.stopBit = (StopBits)Enum.Parse(typeof(StopBits), cbx_stopBit.TextStr);
        }
        private void cbx_parityBit_SelectedIndexChanged()
        {
            ...
            serial.parity = (Parity)Enum.Parse(typeof(Parity), cbx_parityBit.TextStr);
        }
        ...
        private void btn_send_Clicked()
        {
            try
            {
                ...
                serial.Send(tbx_sendMsg.TextStr.Trim());
            }
            catch (Exception ex)
            {
                Log.SaveError(ex);
        ...
        {
            serial.TrigCmd = tbx_trigCmd.TextStr.Trim();
        }
        private void tbx_scanNum_TextStrChanged(string textStr)
        {
            serial.failNum = Convert.ToInt16(tbx_scanNum.TextStr.Trim());
        }
