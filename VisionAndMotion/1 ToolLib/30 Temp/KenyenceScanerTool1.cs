...
{
    [Serializable]
    internal class KenyenceScanerTool1 : ToolBase
    {
        ...
        internal string Read()
        {
            try
            {
                ...
                Frm_KenyenceScanerTool1.Instance.tbx_output.Text += DateTime.Now.ToString("HH:mm:ss") + "->:  " + result + Environment.NewLine;
        ...
        void serialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            try
            {
                ...
                if (receiveStr != "")
                {
                    Frm_KenyenceScanerTool1.Instance.tbx_output.Text += DateTime.Now.ToString("HH:mm:ss") + "<-:  " + receiveStr + Environment.NewLine;
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (!serialPort.IsOpen)
                {
                  ...
                  //  serialPort.DataReceived += new SerialDataReceivedEventHandler(serialPort_DataReceived);
                    Frm_KenyenceScanerTool1.Instance.tbx_output.Text = "打开串口成功";
                    Frm_KenyenceScanerTool1.Instance.lbl_statu.Text = "已打开";
                    Frm_KenyenceScanerTool1.Instance.lbl_statu.ForeColor = Color.Green;
                }
            }
            catch (Exception ex)
            {
                Frm_KenyenceScanerTool1.Instance.lbl_statu.Text = "未打开";
                Frm_KenyenceScanerTool1.Instance.lbl_statu.ForeColor = Color.Red;
                Log.SaveError(ex);
            }
        }
        /// <summary>
        ...
        internal void Send(string cmd)
        {
            try
            {
                ...
                Frm_KenyenceScanerTool1.Instance.tbx_output.Text += DateTime.Now.ToString("HH:mm:ss") + "->:  " + cmd + Environment.NewLine;
                Application.DoEvents();
            }
            catch (Exception ex)
            {
        ...
        public override void Run(bool updateImage, bool b, string toolName)
        {
            try
            {
                ...
                resultStr = sz;
                if (Machine.machineRunStatu != MachineRunStatu.Running)
                    Frm_KenyenceScanerTool1.Instance.tbx_output.AppendText("扫描到：" + sz + Environment.NewLine);
